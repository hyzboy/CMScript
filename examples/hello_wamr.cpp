...
int main()
{
    std::cout << "=== WAMR Example ===" << std::endl;

    // Initialize WAMR VM
    if (!hgl::wasm::InitializeVM(hgl::wasm::VMType::WAMR))
    {
        std::cerr << "Failed to initialize WAMR" << std::endl;
        return 1;
    }
    ...
    std::cout << "WAMR example completed successfully!" << std::endl;
    return 0;
}
