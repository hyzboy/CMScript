...
namespace hgl
{
    ...
    bool AngelContext::_Run(va_list &va)
    {
        ...
        for(uint i=0;i<param_count;i++)
        {
            ...
            {
                LogInfo(U8_TEXT("%s"),
                    (UTF8String("无法自动识别脚本函数")+UTF8String(func->GetDeclaration())
                     +UTF8String("的第")+UTF8String::numberOf(i+1)
                     +UTF8String("个参数,参数名称：")+UTF8String(param_name)
                     +UTF8String("，请联系开发者已便取得支持.")).c_str());
                va_end(va);
                return(false);
            }
#endif//
            ...
            if((type&0xFFFF0000)==asTYPEID_APPOBJECT        )result=ctx->SetArgObject   (i,va_arg(va,void * ));else
            {
                LogInfo(U8_TEXT("%s"),
                    (UTF8String("无法自动识别脚本函数")+UTF8String(func->GetDeclaration())
                     +UTF8String("的第")+UTF8String::numberOf(i+1)
                     +UTF8String("个参数的类型")+UTF8String::numberOf(type)
                     +UTF8String("，请联系开发者已便取得支持.")).c_str());
                va_end(va);
                return(false);
            }
            ...
            {
                LogInfo(U8_TEXT("%s"),
                    (UTF8String("脚本函数")+UTF8String(func->GetDeclaration())
                     +UTF8String("的第")+UTF8String::numberOf(i+1)
                     +UTF8String("个参数")+UTF8String::numberOf(type)
                     +UTF8String("设置失败，返回值：")+UTF8String::numberOf(result)
                     +UTF8String("，请联系开发者已便取得支持.")).c_str());
                va_end(va);
                return(false);
            }
