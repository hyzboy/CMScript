...
namespace hgl
{
    ...
    namespace
    {
        ...
        {
            GLogInfo(U8_TEXT("%s"),(UTF8String("AngelScript Log: ")+str).c_str());
        }
    }
    ...
    AngelVM::AngelVM()
    {
        ...
        {
            LogError(OS_TEXT("%s"),
                     (OS_TEXT("Create AngelScriptEngine")
                      +OSString::numberOf(ANGELSCRIPT_VERSION)
                      +OS_TEXT(" Failed.")).c_str());
            return;
        }
    ...
    void AngelVM::MessageCallback(const asSMessageInfo *msg)
    {
        ...
        LogInfo(U8_TEXT("%s"),
            (UTF8String("AngelScript Message: ")+UTF8String(msg->section)
             +UTF8String(" (")+UTF8String::numberOf(msg->row)
             +UTF8String(",")+UTF8String::numberOf(msg->col)
             +UTF8String(") : ")+UTF8String(type)
             +UTF8String(" : ")+UTF8String(msg->message)).c_str());
    }
    ...
    bool AngelVM::BindProperty(const char *value_name,void *value_address)
    {
        ...
        LogError(OS_TEXT("%s"),
             (OS_TEXT("Register Global Property return ")
              +OSString::numberOf(result)).c_str());
        return(false);
    }
    ...
    bool AngelVM::BindFunc(const char *func_name,void *func_address)
    {
        ...
        LogError(U8_TEXT("%s"),
             (UTF8String("Register Global Function \"")+func_name
              +UTF8String("\" return ")+UTF8String::numberOf(result)).c_str());
        return(false);
    }
